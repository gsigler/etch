...
func TestRunWithStdin_ClaudeNotOnPath(t *testing.T) {
	origPath := os.Getenv("PATH")
	t.Setenv("PATH", "")
	defer os.Setenv("PATH", origPath)

	err := RunWithStdin("test prompt", t.TempDir())
	if err == nil {
		t.Fatal("expected error when claude is not on PATH")
	}
...
func TestRunWithStdin_WorkDirIsSet(t *testing.T) {
	_, err := exec.LookPath("echo")
	if err != nil {
		t.Skip("echo not on PATH")
	}

	err = RunWithStdin("hello", "/nonexistent-dir-for-test")
	if err == nil {
